...
func _ready() -> void:
	...
	if health_attribute and _parent.modifiers:
		...
		health_attribute.on_empty.connect(_on_health_empty)
		health_attribute.on_deplete.connect(_on_damage_received)
		health_attribute.on_restore.connect(_on_health_restored)
